...
public class RoomUserManager {
    ...
    public void removeUserFromCurrentRoom(GameClient client) {
    	if (client == null)
            return;
    	RoomUser user = getUser(client.getUser().getId());
        if (user == null)
    ...
    public void InventoryItem(GameClient client, int id, int x, int y, double z, int rot, int state, BaseItem baseItem, String type) {
   	 	room.getRoomItemManager().InventoryItems(client, id, x, y, z, rot, state, baseItem, type);
    }
    
    public void furniaddInventory(GameClient client, int itemId) {
        	Room rooms = client.getUser().getCurrentRoom();
        	RoomItem item = room.getRoomItemManager().getItem(itemId);
        	if (rooms != null) {
       	 		if(item instanceof WallItem) {
       	 			String type = "W";
       	 			client.sendMessage(new InventarComposer(itemId, item.getBaseItem().getBaseId(), item.getState(), type));
       	 		} else {
       	 			String type = "F";
       	 			client.sendMessage(new InventarComposer(itemId, item.getBaseItem().getBaseId(), item.getState(), type));
       	 		}
    ...
    public void InventoryGiven(GameClient client, boolean yes){
    	client.getUser().setInventar(yes);
    }
